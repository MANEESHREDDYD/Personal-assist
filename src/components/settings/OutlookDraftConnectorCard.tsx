...
export function OutlookDraftConnectorCard() {
  ...
  async function fetchStatus() {
    try {
      const res = await fetch("/api/integrations/outlook-draft/status");
  ...
  async function handleDisconnect() {
    setLoading(true);
    try {
      await fetch("/api/integrations/outlook-draft/disconnect", { method: "POST" });
      await fetchStatus();
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="glass-card p-6 rounded-2xl border border-white/10 hover:border-white/20 transition-all">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-white">Outlook Draft Creation</h3>
      ...
      <div className="text-sm text-gray-400 border-t border-white/5 pt-4">
        <p className="mb-2 text-amber-300 flex items-start gap-2 bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
          ...
          This permission allows creating/updating mailbox drafts. Personal Assist only
          creates drafts and never sends emails. Provider-side draft creation requires
          broader OAuth permissions than read-only access.
        </p>
        <p className="mb-3 text-green-300 flex items-start gap-2 bg-green-500/10 p-3 rounded-lg border border-green-500/20">
          ...
          No-send guarantee: this connector only creates a draft via Microsoft Graph. You
          must review and send manually from Outlook. The read-only Outlook Mail connector
        ...
        {!status?.redirectConfigured && (
          <p className="text-yellow-400 flex items-center gap-2 mb-2">
            <Settings size={16} /> Missing MICROSOFT_OUTLOOK_DRAFT_REDIRECT_URI
          </p>
        )}
        ...
        {!status?.connected && status?.configured && status?.encryptionKeyPresent && (
          <div className="mt-4 pt-4 border-t border-white/5">
            <a
              href="/api/integrations/outlook-draft/start"
            ...
            >
              <PenSquare size={16} /> Connect Outlook Draft
            </a>
          </div>
        )}
      </div>
